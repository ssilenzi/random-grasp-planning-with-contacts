function plot_link(T_init, T_final, r, is_end_effector, RGB, axis)
...
if is_end_effector
    ...
    [sx, sy, sz] = sphere;
    hold on
    surf(sz.*r + T_init(3,4), sx.*r + T_init(1,4), sy.*r + T_init(2,4), ...
        'edgecolor','none', 'FaceColor', RGB);
    
if ~is_end_effector
    surf(sz.*r + T_final(3,4), sx.*r + T_final(1,4), ...
...
quiver3(p(3), p(1),p(2), m_axis(3), m_axis(1), m_axis(2), scale, ...
