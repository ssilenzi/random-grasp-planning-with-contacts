% V_optimal_mincon_grad
%
%   Computes the minimum of the fucntion V using the MATLAB supplied
%   fmincon, using also the formula for the gradient of the function
%
...
%
function [fc_opt, y_opt, V_opt, V_0, exitflag, output_info, elapsed_time ,  sigma_leq , lambda,grad,hessian] = V_optimal_mincon_grad( f_c0, normals, mu, f_min, f_max , cf_dim , E )
%
time_0 = clock;
y0 = zeros(size(E,2),1) ;
...
options = optimoptions(@fmincon,'GradObj','on', ...%'Algorithm','trust-region-reflective',
                 'TolFun',1e-20,'TolX',1e-20, 'MaxIter',1000,'MaxFunEvals',1000);
    %'TolFun',1e-30,'TolX',1e-60, 'MaxIter',2000,'MaxFunEvals',2000);
...
[y_opt,V_opt,exitflag,output_info, lambda,grad,hessian] = fmincon(@V_mincon, y0,[],[],[],[],[],[],@nonlcon,options) ;
                   %[y_opt,V_opt,exitflag,output_info] = fminunc(@V_minunc, y0, options);
    function [V_min, V_grad_y ] = V_mincon(y)
        f_c_loop = f_c0 + E*y ;
