% D_V_tot
%
%   Computes the derivative of the function V (to be minimized) for the current contact force distribution.
%   Friction cone, minimum and maximum normal force are considered
%
%   Syntax: [ D_V_tot ] = D_V_y( f_c, normals, mu, f_min, f_max , cf_dim, E  )
%
%   Inputs:
...
%
% Outputs:
% D_V_tot_out is the derivative (gradient) of the fucntion V 
%
function [ D_V_tot_out ] = D_V_tot( f_c, normals, mu, f_min, f_max , cf_dim, E  )
%
...
for i = 1 : length(cf_dim)
    ...
    % 
    D_V_tot_out(:,1) = D_V_tot_out(:,1)+ D_V_i( f_ci, n_i, mu_i, f_min_i, f_max_i, E_i  ) ; %   V(i*2-1:2*i,1)
end
%
end
%
