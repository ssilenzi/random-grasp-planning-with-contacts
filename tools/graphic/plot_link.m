...
handle(1) = surf([X1(3,:); X2(3,:)], [X1(1,:); X2(1,:)], [X1(2,:); X2(2,:)], ...
                'edgecolor','none', 'FaceColor', RGB);
    
[sx, sy, sz] = sphere;
hold on
handle(2) = surf(sz.*r + T_init(3,4), sx.*r + T_init(1,4), sy.*r + T_init(2,4), ...
                'edgecolor','none', 'FaceColor', RGB);
...
if ~is_end_effector
    ...
    X2 = p1 + p_delta + R*[0*cos(theta); 
        0*sin(theta); 
        zeros(size(theta))];
    handle(3) = surf([X1(3,:); X2(3,:)], [X1(1,:); X2(1,:)], [X1(2,:); X2(2,:)], ...
                    'edgecolor','none', 'FaceColor', RGB);
end
...
handle(4) = quiver3(p(3), p(1),p(2), m_axis(3), m_axis(1), m_axis(2), scale, ...
