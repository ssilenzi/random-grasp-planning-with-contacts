function [exit,nodes_out,edges_out] = ...
    implement_direct_twist_real_robot(node_s,environment, ...
...
for i = 1:length(spaced_vec)
    ...
    [bool, coll_type] = check_collisions_box(box_next, environment, coll_res);
    if bool == true
        if verbose
    ...
    if ~success || ~isempty(self_coll_pair_id) || ~isempty(world_coll_pair_id)
        if verbose
            disp('DIRTWIST - Collision hand env or bad IK detected while moving hand');
        end
        break;
    else
        if verbose
