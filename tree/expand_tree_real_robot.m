...
% EXPAND TREE REAL ROBOT - expands the tree by implementing spawning, 
% positioning, release or moving. Here positioning is not alone and done
% with moving. This one uses robotic system toolbox collisions check and
% robot description
%   Inputs:
...
%     disp('Currently processing node: '); disp(ID_s);
    ...
    if Cont_h_s == false    % IMPLEMENT POSITIONING
        ...
        if prev_pos_s || less_3_env_cont || rand_num > p_release % MOVING
            ...
            [exit, nodes_out, edges_out] = ...
                implement_moving_real_robot(node_s, environment, ...
                force_params, edge_types, edge_weights, ...
                target, n_nodes, dt_max);            
            ...
            [exit, nodes_out, edges_out] = ...
                implement_release_real_robot(node_s, environment, ...
                force_params, edge_types, edge_weights, ...
                target, n_nodes);
