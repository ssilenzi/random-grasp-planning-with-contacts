...
%
function [fc_opt, y_opt, V_opt, V_0, exitflag, output_info, elapsed_time, sigma_leq ,grad,hessian] = V_optimal_minunc( fc_0, normals, mu, f_min, f_max , cf_dim , E )
%
time_0 = clock;
y0 = zeros(size(E,2),1) ;
...
options = optimoptions(@fminunc,'Algorithm','quasi-newton', ...
    'TolFun',1e-20,'TolX',1e-20, 'MaxIter',1000,'MaxFunEvals',1000); %'quasi-newton');
[y_opt,V_opt,exitflag,output_info, grad,hessian] = fminunc(@V_minunc, y0, options);
    function V_min = V_minunc(y)
        f_c_loop = fc_0 + E*y ;
