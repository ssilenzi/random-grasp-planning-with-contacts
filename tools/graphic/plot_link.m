...
handl(1) = surf([X1(1,:); X2(1,:)], [X1(2,:); X2(2,:)], [X1(3,:); X2(3,:)], ...
    'edgecolor','none', 'FaceColor', RGB);

[sx, sy, sz] = sphere;
hold on
handl(2) = surf(sz.*r + T_init(1,4), sx.*r + T_init(2,4), sy.*r + T_init(3,4), ...
    'edgecolor','none', 'FaceColor', RGB);

if ~is_end_effector
    handl(3) = surf(sz.*r + T_final(1,4), sx.*r + T_final(2,4), ...
        sy.*r + T_final(3,4), 'edgecolor','none', 'FaceColor', RGB);
else
    X1 = X2;
    X2 = p1 + p_delta + R*[0*cos(theta);
        0*sin(theta);
        zeros(size(theta))];
    handl(3) = surf([X1(1,:); X2(1,:)], [X1(2,:); X2(2,:)], [X1(3,:); X2(3,:)], ...
        'edgecolor','none', 'FaceColor', RGB);
end
...
handl(4) = quiver3(p(1), p(2),p(3), m_axis(1), m_axis(2), m_axis(3), scale, ...
