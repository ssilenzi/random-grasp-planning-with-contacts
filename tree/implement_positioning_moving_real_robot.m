function [exit,nodes_out,edges_out] = ...
    implement_positioning_moving_real_robot(node_s,environment, ...
    force_params,edge_types,edge_weights,target,n_nodes,dt_max,T_rand)
...
for i = 1:n_try
    ...
    robot_f.get_starting_config(Cp_h_f, Cn_h_f, Co_s, box_s);

	% Moving robot to contacts
    [robot_f, success] = move_franka_to_points(robot_f,Cp_h_f);
    [~, self_coll_pair_id, world_coll_pair_id] = ...
        robot_f.build_check_collisions(box_f, environment);
    ...
    % Moving robot to contacts
    [robot_f1, success] = move_franka_to_points(robot_f1,Cp_h_f1,wrist1);
    [~, self_coll_pair_id, world_coll_pair_id] = ...
        robot_f1.build_check_collisions(box_f1, environment);
