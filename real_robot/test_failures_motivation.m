...
%% Comparing active internal forces subspace by making gripper floating
% J01m = J01;
% J01m(:,8) = J01m(:,8) + J01m(:,9);
% J01m(:,9) = [];

% Making a floating franka gripper
% J01f = J01; 
% J01f(:,7) = [];
% J01f(1:8,1:6) = zeros(8,6); 
% J01f(1:3,1:3) = eye(3); J01f(4,6) = 1;
% J01f(5:7,1:3) = eye(3); J01f(8,6) = 1;

[Jm,Jf] = get_J_float_franka(franka,J01,Cp_h0,Cn_h0);

[Em, dQm, dUm] = basis_active_internal_forces_2(G01, Jm, K01);
[Ef, dQf, dUf] = basis_active_internal_forces_2(G01, Jf, K01);
...
% Checking the optimization with fmincon
y0m = zeros(size(Em,2),1);
y0f = zeros(size(Ef,2),1);

tic
[fc_optm, y_optm, cost_optm, cost_initm, exitflagm, outputm, elapsed_timem, ...
    sigma_leqm] = solve_constraints_full_mincon_correct(fp01, ...
    Em, y0m, normals01, mu_vect01, f_min_vect01, f_max_vect01, ...
    m_min_vect01, m_max_vect01, cf_dim_tot01, Delta);
toc

disp('The following do not verify the constraints ');
disp(find(sigma_leqm > Delta));
disp('The sum of the forces is ');
disp(norm(we + G01*fc_optm));

tic
[fc_optf, y_optmf, cost_optf, cost_initf, exitflagf, outputf, elapsed_timef, ...
    sigma_leqf] = solve_constraints_full_mincon_correct(fp01, ...
    Ef, y0f, normals01, mu_vect01, f_min_vect01, f_max_vect01, ...
    m_min_vect01, m_max_vect01, cf_dim_tot01, Delta);
toc

disp('The following do not verify the constraints ');
disp(find(sigma_leqf > Delta));
disp('The sum of the forces is ');
disp(norm(we + G01*fc_optf));
